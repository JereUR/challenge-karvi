  ...
  return (
    <main className="flex min-h-screen flex-col space-y-2 md:space-y-3">
      ...
      <div className="flex w-full grow md:gap-5 p-2 md:p-5">
        <Sidebar />
        <MainContent gridMode={gridMode} setGridMode={setGridMode} />
      </div>
    </main>
  )
}
...
function MainContent({ gridMode, setGridMode }: MainContentProps) {
  ...
  const { toggleFavorite, favorites } = useFavoritesCars(currentPage, ITEMS_PER_PAGE)
